...
function hena(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(alazizy) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '  انت حمار يابني ازاي هرفع نفسي انت متعرفش انا بوت مين ولا اي ..😹😹♥️', 1, 'md') 
return false  
end  
if tonumber(result.sender_user_id_) == tonumber(DevId) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, '  بس ياكلب نت عبيط ده مطور يعني ليه وضعوو ازاي ارفعو خنزير ياخنزير..😹♥️', 1, 'md') 
return false
end 
if tonumber(result.sender_user_id_) == tonumber(1183747742) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, '  دا العزايزي شخصيا يامجنون ازاي ارفعه خنزير😹♥️', 1, 'md') 
return false
end 
if DevAbs:sismember(alazizy.."Abs:AbsConstructor:"..msg.chat_id_,result.sender_user_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '  بس ياكلب نت عبيط ده مطور يعني ليه وضعوو ازاي ارفعو خنزير ياخنزير..😹♥️', 1, 'md') 
...
local msg_id = msg.id_/2097152/0.5 
https.request('https://ccccxcc.ml/David/alazizy.php?Put=Search&TokenBot='..TokenBot..'&ChatId='..msg.chat_id_..'&UserId='..msg.sender_user_id_..'&Text='..URL.escape(Text)..'&MsgId='..msg_id..'&Name='..DirName)
end
if text and text:match('^بحث (.*)$') and not DevAbs:get(alazizy.."Abs:WhyTube"..msg.chat_id_) then            
local Text = text:match('^بحث (.*)$') 
local msg_id = msg.id_/2097152/0.5 
https.request('https://ccccxcc.ml/alazizy/Api.php?Put=Search&TokenBot='..TokenBot..'&ChatId='..msg.chat_id_..'&UserId='..msg.sender_user_id_..'&Text='..URL.escape(Text)..'&MsgId='..msg_id..'&Name='..DirName)
end
--     Source alazizy     --
if SecondSudo(msg) then
